...
class AttributionMethod(object):
    ...
    def __init__(self, T, X, model, keras_learning_phase=None):
...
class GradientBasedMethod(AttributionMethod):
    ...
    def get_symbolic_attribution(self):
        return tf.GradientTape(self.T, self.X)
    ...
    #     results = self.model(xs,is_training = False)
    #     # xs = tf.convert_to_tensor(xs)
    #     # with tf.GradientTape() as tape:
    #     #     tape.watch(xs)
    #     #     prediction = self.model(xs,is_training = False)

    #     # results = tape.gradient(prediction, xs).numpy()
    #     # print(results.shape)
...
class PerturbationBasedMethod(AttributionMethod):
    """
       ...
       """
    def __init__(self, T,X, model, keras_learning_phase):
        super(PerturbationBasedMethod, self).__init__( T, X, model, keras_learning_phase)
...
class Saliency(GradientBasedMethod):
    ...
    def get_symbolic_attribution(self):
        ...
        # with tf.GradientTape() as tape:
        #     tape.watch(self.X)
        #     for g in tape.gradient(self.T, self.X):
        #         res.append(tf.abs(g))
        # return res


        # return [tf.abs(g) for g in tf.gradient(self.T, self.X)]
        pass

    def run(self, xs, ys=None, batch_size=None):
    # self._check_input_compatibility(xs, ys, batch_size)
    # results = self._session_run(self.explain_symbolic(), xs, ys, batch_size)
        results = self.model(xs,training = False)
        xs = tf.convert_to_tensor(xs)
        with tf.GradientTape() as tape:
            tape.watch(xs)
            prediction = self.model(xs,training = False)

        results = tf.abs(tape.gradient(prediction, xs)).numpy()
        # print(results.shape)
        return results
...
class GradientXInput(GradientBasedMethod):
    ...
    def get_symbolic_attribution(self):
        ...
        # with tf.GradientTape() as tape:
        #     tape.watch(self.X)
        #     for g,x in zip (tape.gradient(self.T, self.X), self.X if self.has_multiple_inputs else [self.X]):
        #         res.append(g*x)
        # return res

        # return [g * x for g, x in zip(
        #     tf.gradient(self.T, self.X),
    ...
    # results = self._session_run(self.explain_symbolic(), xs, ys, batch_size)
        ...
        results = (tape.gradient(prediction, xs) * xs).numpy()
        # print(results.shape)
        return results
...
class IntegratedGradients(GradientBasedMethod):
    ...
    def __init__(self, T, X, model, keras_learning_phase, steps=100, baseline=None):
        self.steps = steps
        self.baseline = baseline
        super(IntegratedGradients, self).__init__(T, X, model, keras_learning_phase)
    ...
    def run(self, xs, ys=None, batch_size=None):
        ...
        for alpha in list(np.linspace(1. / self.steps, 1.0, self.steps)):
            ...
            xs_mod = tf.convert_to_tensor(xs_mod,dtype=tf.float32)
            ...
            with tf.GradientTape() as tape:
                tape.watch(xs_mod)
                prediction = self.model(xs_mod,training = False)

            _attr = (tape.gradient(prediction, xs_mod)).numpy()
...
#     def __init__(self, T, X, model, keras_learning_phase, epsilon=1e-4):
#         assert epsilon > 0.0, 'LRP epsilon must be greater than zero'
#         global eps
#         eps = epsilon
#         super(EpsilonLRP, self).__init__(T, X, model, keras_learning_phase)
...
#     def __init__(self, T, X, model, keras_learning_phase, baseline=None):
#         self.baseline = baseline
#         super(DeepLIFTRescale, self).__init__(T, X, model, keras_learning_phase)
...
class Occlusion(PerturbationBasedMethod):
    ...
    def __init__(self, T, X, model, keras_learning_phase, window_shape=None, step=None):
        super(Occlusion, self).__init__(T,X, model, keras_learning_phase)
        if self.has_multiple_inputs:
    ...
    def run(self, xs, ys=None, batch_size=None):
        ...
        if ys is None:
            ys = np.ones([1,] + self.Y_shape[1:])
        # Compute original output
        # xs = tf.convert_to_tensor(xs,dtype=tf.float32)
        eval0 = self.model(xs)*ys
        ...
        for i, p in enumerate(idx_patches):
            ...
            delta = ((eval0 - self.model(masked_xs)*ys)/eval0).numpy()
...
class ShapleySampling(PerturbationBasedMethod):
    ...
    def __init__(self, T, X, model, keras_learning_phase, samples=5, sampling_dims=None):
        super(ShapleySampling, self).__init__(T, X, model, keras_learning_phase)
        if self.has_multiple_inputs:
            raise RuntimeError('Multiple inputs not yet supported for perturbation methods')
        dims = len(X.shape)
        ...
        for r in range(self.samples):
            ...
            for i in p:
                if y is None:
                    ...
                    y = self.model(x.reshape(xs_shape),training =False)


                x[:, i] = 0
                y0 = self.model(x.reshape(xs_shape))
...
class DeepExplain(object):
    ...
    def __init__(self, graph=None, model=None):
        self.method = None
